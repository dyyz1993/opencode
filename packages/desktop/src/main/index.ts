...
import { checkForUpdates, checkUpdate, installUpdate, setupAutoUpdater } from "./updater"
...
const onboardingTestRoot = (() => {
  ...
  for (const dir of ["data", "config", "cache", "state", "desktop", "session"]) {
...
registerIpcHandlers({
  ...
  runUpdater: async (alertOnFail) => checkForUpdates(alertOnFail, shutdownSidecars),
  checkUpdate: async () => checkUpdate(),
  installUpdate: async () => installUpdate(shutdownSidecars),
...
function setupApp() {
  ...
  for (const signal of ["SIGINT", "SIGTERM"] as const) {
    process.on(signal, () => {
      void shutdownSidecars().finally(() => app.exit(0))
    })
  }
...
function wireMenu() {
  ...
  createMenu({
    ...
    checkForUpdates: () => {
      void checkForUpdates(true, shutdownSidecars)
    },
...
async function shutdownSidecars() {
  await killSidecar()
  wslServers.stopAll()
}

function relaunchApp() {
  void shutdownSidecars().finally(() => {
    app.relaunch()
    app.exit(0)
  })
}
